...
object TestsHelper extends FunSuite {
  ...
  def shouldSucceed[T](rule: (String, Int) => ParseResult[Any,_])
    ...
    tests foreach {
      x => rule(x.in, 0) match {
        case Success(result) =>
          assert(result == x.res, s"on ${x.in} expected ${x.res}, got ${result}")
        case Failure(msg) => fail(s"""Didn't succeed : ${msg} on "${x.in}" """)
      }
    }
  }
  ...
  def compare(s1: String, s2: String) = {
    s1.zip(s2).zipWithIndex.foreach { case ((a1, a2), i) =>
      if (a1 != a2)
        fail(s"error at $i : ${s1.substring(i,i + 10)} != ${s2.substring(i,i + 10)}")
    }
  }
