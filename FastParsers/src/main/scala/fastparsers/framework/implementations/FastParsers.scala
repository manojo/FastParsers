...
import fastparsers.framework.ruleprocessing.{
  RuleCombiner, ParseRules, RulesInliner, RulesTransformer, ParseQuery
}
...
class FastParsersImpl(val c: Context)
    extends BaseImpl
...
/**
 * The interface that takes care of running the ParseQuery transform
 * on top of everything
 */
object FasterParsers
    extends BaseParsers[Char, String]
    with RepParsers
    with TokenParsers[String]
    with FlatMapParsers {
  def FastParser(rules: => Unit): FinalFastParserImpl =
    macro FasterParsersImpl.FastParser
}
...
class FasterParsersImpl(val c: Context)
    extends BaseImpl
    with RulesTransformer
    with ParseQuery
    with ParseRules
