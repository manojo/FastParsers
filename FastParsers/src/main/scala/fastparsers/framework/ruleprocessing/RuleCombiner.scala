...
trait RuleCombiner extends ReduceRules {
  ...
  def combine(rules: HashMap[String, RuleInfo]) = {
    ...
    /**
     * no joke : see http://stackoverflow.com/questions/14370842/getting-a-structural-type-with-an-anonymous-classs-methods-from-a-macro
     */
    ...
    val methodsEmpty = rules.keySet map { k =>
      ...
      rule.params match {
        case Nil =>
          q"""@scala.annotation.compileTimeOnly("can't be used outside of FastParser")
              def $ruleName[..${rule.typeParams}]: fastparsers.parsers.Parser[${rule.typ}] = ???"""
        case params =>
          q"""@scala.annotation.compileTimeOnly("can't be used outside of FastParser")
              def $ruleName[..${rule.typeParams}](..${rule.params}): fastparsers.parsers.Parser[${rule.typ}] = ???"""
      }
    }
