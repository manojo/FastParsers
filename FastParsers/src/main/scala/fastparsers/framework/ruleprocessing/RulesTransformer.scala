...
/**
 * Trait used to transform rules so that calls to other rules and foreign calls
...
trait RulesTransformer extends MapRules { self: TreeTools with ParseInput =>
  ...
  def transformRules(rulesMap: HashMap[String, RuleInfo]) = {
    ...
    for (k <- rulesMap.keys) transformRule(k)
    expandedRulesMap
  }
 ...
 def transformRuleCalls(tree: c.Tree,
    ...
    /**
     * match on all diffent trees we can have in a ``FastParser` context
     */
    tree match {
      case q"if($c) $a else $b" => q"""
        if ($c) ${transformRuleCalls(a, enclosingRule,rulesMap,expandedRules,rulesPath)}
        else ${transformRuleCalls(b, enclosingRule,rulesMap,expandedRules,rulesPath)}"""
